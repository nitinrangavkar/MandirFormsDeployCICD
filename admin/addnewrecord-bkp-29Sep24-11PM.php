...
if(isset($_POST['reg_user']))
{
    ...
    {
		$isSaveTrue = false;
        echo '<script>alert("कृपया प्रसादासाठी बंधू किंवा भगिनी संख्या नमूद करा ");</script>';
    }
    ...
    if ($total_seva_bandhu != ((int)$breakfast_brothers + (int)$lunch_brothers + (int)$dinner_brothers) && $isSaveTrue) {
		$isSaveTrue = false;
        echo '<script>alert("कृपया प्रसादासाठी बंधू संख्या तपासा ");</script>';
    } elseif ($total_seva_bhagini != ((int)$breakfast_sisters + (int)$lunch_sisters + (int)$dinner_sisters) && $isSaveTrue) {
		$isSaveTrue = false;
        echo '<script>alert("कृपया प्रसादासाठी भगिनी संख्या तपासा");</script>';
    } else {
			...
			if($remaining_bandhu >= $total_seva_bandhu)
			{
				if($remaining_baghini >= $total_seva_bhagini)
				{
								...
								if (mysqli_query($db, $sql)) {
									...
									echo '<script>alert("माहिती यशस्वीपणे जमा झाली ");</script>';
								} else {
				...
				{
					echo '<script>alert("भगिनी संख्या हि बाकी असलेल्या भगिनी संख्येपेक्षा जास्त आहे ");</script>';
				}
									
			}//close bandhu count exist
			else
			{
				echo '<script>alert("बंधू संख्या हि बाकी असलेल्या बंधू संख्येपेक्षा जास्त आहे ");</script>';
			}
		}
    }
}
  ...
  <main id="main" class="main">
    <div class="row">
		...
		<div>
			<p>
                    आमच्या <b>
                            <input type="text" name="branch" class="form-control" list="shakha" required />
                            <datalist id="shakha">
                                <?php 
                    ...
                    शाखेतून, 
                    <input type="number" name="brothers" min="0" max="100" value="<?php echo $_SESSION['brothers'];?>" readonly> बंधू व 
                    <input type="number" name="sisters" min="0" max="100" value="<?php echo $_SESSION['sisters'];?>" readonly> भगिनी एकूण 
                    <input type="number" name="total_people" min="0" max="100" value="<?php echo $_SESSION['total_people'];?>" readonly> मंडळी
		...
		<div>
            ...
            <table border="1" id="seva" class="tableStyle">
                ...
                <!-- add seva script -->
                <script>
					function IsSevaAlradyAdded(seva) {
						debugger;
						var tblAddedSeva = document.getElementById("tblSeva");
						if(tblAddedSeva != null) {
							for (var i = 1, row; row = tblAddedSeva.rows[i]; i++) {
								if(row.cells[0].innerText.replace(/\s/g,'').trim() == seva.replace(/\s/g,'').trim())
									return true;
							}
						}
						return false;
					}
					function ResetSevaWithCount() {
						document.getElementById("seva_type").options.selectedIndex = 0;
                    ...
                    function addSeva() {
                        ...
                        let sevaSisters = document.getElementById("seva_sisters").value || 0;
                            ...
                            if (!sevaType) {
                                alert("कृपया प्रथम सेवा निवडा ");
                                return;
                            } else if (sevaBrothers == 0 && sevaSisters == 0) {
								alert("निवडलेल्या सेवेसाठी कृपया बंधू किंवा भगिनी संख्या नमूद करा ");
								return;
							} else if (IsSevaAlradyAdded(sevaType)) {
								ResetSevaWithCount();
								alert("निवडलेली सेवा आधीच नमूद झाली आहे, कृपया वरील तपशील तपासा ");
								return;
							}
                            ...
                            $.ajax({
                                    ...
                                    success: function(response) {
                                        ...
                                        fetchUpdatedSevaData();
										ResetSevaWithCount();
										alert("सेवेसाठी बंधू आणि भगिनी संख्या यशस्वीरीत्या जमा झाली आहे");
                                    },
                                    error: function() {
                                        alert("तांत्रिक बिघाडामुळे माहिती जमा झाली नाही");
                                    }
                                });
                            }
                            ...
                            function fetchUpdatedSevaData() {
                                $.ajax({
                                    ...
                                    error: function() {
                                        alert("तांत्रिक बिघाडामुळे माहिती दाखवू शकत नाही");
                                    }
                                });
                            }
                </script>
            ...
            <center><button type="submit" name="reg_user" class="btn btn-success">जमा करा</button></center>
        </form>
        <form action="#" method="post">
            <center><button type="submit" name="clear_all" class="btn btn-secondary">खोडा </button></center>
        </form>
    </div>
</div>
